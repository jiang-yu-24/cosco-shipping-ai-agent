...
def run_agent(user_query: str, chat_history: Optional[List[Dict[str, Any]]] = None) -> str:
    ...
    system_prompt = {
        "role": "system",
        "content": (
            "你是一个专业的央国企数字化项目 AI 助理，名字叫「远航助手」。"
            "你的职责是帮助用户处理航运业务、编写文档、生成 PDF 等。"
            "你可以调用提供的工具函数来获取实时数据。"
            "每次调用工具后，你会收到一个反思提示——请认真评估工具返回的结果。"
            ""
            "【PDF 生成规则——必须严格遵守】"
            "当用户要求生成 PDF 时，根据用户原话选择 doc_type："
            "- 提到「船期」「确认函」「航运安排」→ doc_type='schedule'"
            "- 提到「报告」「分析」「周报」「月报」→ doc_type='report'"
            "- 提到「通知」「函」「请示」「批复」「决定」「意见」→ doc_type='official'"
            "- 提到「说明书」「教程」「手册」「总结」「指南」「说明」→ doc_type='generic'"
            "- 提到「方案」「项目建设」「立项」「规划」「可研」「实施计划」「建设方案」→ doc_type='proposal'"
            "当 doc_type='proposal' 时，必须将用户的项目名称填入 route 参数，申报单位填入 consignor 参数。"
            "content 参数必须严格按照以下模板格式组织，每个章节都必须包含：\n"
            "一、项目概述\n项目名称、申报单位、项目负责人、项目类型、建设周期、估算总投资\n"
            "二、项目背景与必要性\n政策依据、业务现状、必要性论证\n"
            "三、建设目标\n总体目标、阶段性目标、量化指标\n"
            "四、建设内容与方案\n功能模块清单、业务流程设计、数据资源规划、配套基础设施\n"
            "五、技术方案\n技术路线、系统架构设计、关键技术说明、安全方案、国产化适配\n"
            "六、实施计划\n实施策略、进度安排、里程碑节点、人员组织\n"
            "七、投资估算\n软件费用、硬件费用、实施服务费、运维费用、总计\n"
            "八、效益分析\n经济效益、管理效益、社会效益、投资回收期\n"
            "九、风险分析与应对\n（表格：风险类别、风险描述、影响程度、发生概率、应对措施）\n"
            "十、组织保障\n领导小组、项目团队、沟通机制、质量保障措施\n"
            "注意：content 中的章节标题必须以'一、二、三、...'开头，不得省略任何章节。"
    ...
    while tool_round < _MAX_TOOL_CALL_ROUNDS:
        ...
        messages.append({
            ...
            "content": (
                ...
                "2. 是否需要调整参数重新查询？\n"
                "3. 当前信息是否足够回答用户？\n"
                "如果还需要更多信息，请继续调用工具；如果足够，请直接给出最终回答。"
            ),
        })
